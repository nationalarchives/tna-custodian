...
data "template_file" "iam_access_key_delete_lambda_common_policy" {
  template = file("./modules/iam/templates/common/lambda_common_policy.json.tpl")
}

resource "aws_iam_policy" "iam_access_key_delete_lambda_common_policy" {
  count  = var.iam_access_key_delete == true ? 1 : 0
  name   = "${upper(var.project)}CustodianIamAccessKeyDeleteLambdaCommon${title(local.environment)}"
  policy = data.template_file.iam_access_key_delete_lambda_common_policy.rendered
}

resource "aws_iam_role_policy_attachment" "iam_access_key_delete_lambda_common_policy_attach" {
  count  = var.iam_access_key_delete == true ? 1 : 0
  role       = aws_iam_role.iam_access_key_delete_assume_role.*.name[0]
  policy_arn = aws_iam_policy.iam_access_key_delete_lambda_common_policy.*.arn[0]
}
